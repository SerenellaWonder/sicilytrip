...
export class PartnerSolutionClient {
  ...
  constructor(
    ...
    this.baseUrl = this.config.get<string>('PARTNERSOLUTION_BASE_URL') ?? '';
  ...
  private async request<T>(
    ...
    try {
      ...
      if (err.response) {
        ...
        this.logger.error(JSON.stringify(err.response.data, null, 2));
      }

      throw new HttpException(
        err.response?.data ?? 'Partner Solution Error',
