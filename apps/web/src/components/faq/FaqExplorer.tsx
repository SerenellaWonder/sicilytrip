    ...
    return sections
      ...
      .map((section) => ({
        ...
        questions: section.questions.filter((item) =>
          normalize(`${item.question} ${item.answer}`).includes(
            normalizedQuery,
          ),
        ),
      }))
    ...
    <div className="mt-14">
      <div className="rounded-[24px] bg-white p-4 shadow-[0_12px_40px_rgba(13,35,64,0.05)] sm:p-5">
        <label className="flex items-center gap-3">
          ...
          <span className="sr-only">Cerca nelle domande frequenti</span>
          <input
            type="search"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Cerca una domanda, ad esempio cancellazione..."
      ...
      <div className="mt-5 flex flex-wrap gap-2" aria-label="Categorie FAQ">
      ...
      <p className="mt-6 text-xs text-slate-400" aria-live="polite">
        {resultCount}{" "}
        {resultCount === 1 ? "risposta trovata" : "risposte trovate"}
      </p>
      ...
      {resultCount ? (
        ...
        <div className="mt-5 rounded-[24px] bg-white px-6 py-12 text-center">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Prova con parole più generiche oppure seleziona un&apos;altra
            categoria.
          </p>
        </div>
      )}
